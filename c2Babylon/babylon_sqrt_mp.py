...
def babylon_sqrt_mp(S, precision_dps=15, print_guesses=False, verbose=False):
    ...
    if abs(mpm.fsub(mpm.power(x[-1], 2), S)) >= tau:
        ...
        guesses += 1
        x.append(mpm.fdiv(mpm.fadd(x[-1], mpm.fdiv(S, x[-1])), 2))
...
def num_iter(tau, S, x0):
    """Estimate the number of iterations."""
    d = abs(mpm.log10(tau))
    mu = mpm.fdiv(1, mpm.fmul(2, mpm.sqrt(S)))
        ...
        if e0 != mpm.mpmathify(0):
            return int(
                mpm.ceil(
                    mpm.log(
                        abs(
                            mpm.log10(mpm.fmul(mu, mpm.power(10, -d)))
                            / mpm.log10(mpm.fmul(mu, e0))
                        ),
                        2,
                    )
                )
...
def main():
    ...
    x = babylon_sqrt_mp(S, prec, True, True)
    print(
